...
function MovieList({ selectedYear, selectedGenre, toggleSidebar }) {
  ...
  }, [API_KEY, selectedYear, selectedGenre]);

  const renderSkeletonCards = () => {
    const numberOfSkeletons = 20; // Five per row, four rows
    return Array.from({ length: numberOfSkeletons }).map((_, index) => (
      <SkeletonCard key={index} />
    ));
  };

  const tmdbLink = (id) => `https://www.themoviedb.org/movie/${id}`;

  return (
    <div className="movie-list">
      <div className='offCanvas'>
        <h2 className='ga-maamli-regular'>
          {selectedYear ? `${selectedYear} Movies` : selectedGenre ? `${selectedGenre} Movies` : 'Featured Movies'} :-
        </h2>
      ...
      <div className="movies-grid">
        {loadingFeatured
          ...
          : featuredMovies.map(movie => (
              <MovieCard key={movie.id} movie={movie} playLink={tmdbLink(movie.id)} />
            ))
        }
      </div>
